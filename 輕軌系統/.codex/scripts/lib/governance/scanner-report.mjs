...
const bundleRefs = Object.freeze({
  ...
  evidenceSchemaRegistry: ".codex/governance/evidence-schema-registry.yaml",
  referencedEvidencePolicy: ".codex/governance/referenced-evidence-validation-policy.yaml",
...
export async function loadScannerContractBundle(projectRoot) {
  ...
  const evidenceSchemaRegistrySha256 = sha256(bytes.evidenceSchemaRegistry);
  const referencedEvidencePolicySha256 = sha256(bytes.referencedEvidencePolicy);
  if (documents.contract.evidence_schema_registry_ref !== bundleRefs.evidenceSchemaRegistry || documents.contract.evidence_schema_registry_sha256 !== evidenceSchemaRegistrySha256 || schemaSet.evidence_registry_sha256 !== evidenceSchemaRegistrySha256) violations.push("SCAN_CONTRACT pinned evidence-schema registry binding mismatch.");
  if (documents.contract.referenced_evidence_policy_ref !== bundleRefs.referencedEvidencePolicy || documents.contract.referenced_evidence_policy_sha256 !== referencedEvidencePolicySha256) violations.push("SCAN_CONTRACT pinned referenced-evidence policy binding mismatch.");
  return {
    ...documents,
    hashes: {
      ...
      evidence_schema_registry_sha256: evidenceSchemaRegistrySha256,
      referenced_evidence_policy_sha256: referencedEvidencePolicySha256
    },
    violations
  };
}
...
export async function buildBootstrapScanReport({ projectRoot, manifestBytes, manifest, startedAt, completedAt = startedAt, executionId = null }) {
  ...
  const payload = {
    ...
    validation_capabilities: {
      static_scan_clean: findings.length === 0,
      referenced_evidence_validation_contract_present: true,
      candidate_binding_complete: includedFiles.length === scannedFiles.length
    },
...
export async function validateBootstrapScanReport({ projectRoot, report, manifestBytes, includedFiles, scannedFiles = includedFiles }) {
  ...
  if (report?.schema_version !== 3) violations.push("SCANNER_REPORT active schema_version must be 3.");
  ...
  const capabilities = report?.validation_capabilities ?? {};
  if (capabilities.static_scan_clean !== ((report?.results?.findings ?? []).length === 0) || capabilities.referenced_evidence_validation_contract_present !== true || capabilities.candidate_binding_complete !== (sortedIncluded.length === sortedScanned.length)) violations.push("SCANNER_REPORT validation capability claims mismatch.");
