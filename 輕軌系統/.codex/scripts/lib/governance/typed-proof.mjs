...
export const VALIDATOR_VERSION = "PHASE1-BOOTSTRAP-4";
...
export function createBootstrapWorkspaceCandidateRecord({ task_id, manifest_sha256, included_file_set_sha256, file_count, scanner_report_sha256, scan_contract_sha256, finding_registry_sha256, canonicalization_config_sha256, binary_oracle_config_sha256, binary_magic_registry_sha256, schema_set_sha256, evidence_schema_registry_sha256, referenced_evidence_policy_sha256, supersedes_candidate_manifest_sha256, supersession_reason, breaking_governance_change = "referenced_evidence_validation_required", generated_at, validator_version = VALIDATOR_VERSION }) {
  const payload = {
    schema_version: 4,
    ...
    schema_set_sha256,
    evidence_schema_registry_sha256,
    referenced_evidence_policy_sha256,
    supersedes_candidate_manifest_sha256,
    supersession_reason,
    breaking_governance_change,
    generated_at,
    validator_version
  };
...
export function validateBootstrapCandidateRecord(record, expected = {}) {
  ...
  const v4BindingFields = [...v3BindingFields, "evidence_schema_registry_sha256", "referenced_evidence_policy_sha256"];
  const supersessionFields = ["supersedes_candidate_manifest_sha256", "supersession_reason", "breaking_governance_change"];
  const bindingFields = record.schema_version === 4 ? v4BindingFields : record.schema_version === 3 ? v3BindingFields : v2BindingFields;
  const required = record.schema_version === 1 ? legacyRequired : [...legacyRequired, ...bindingFields, ...(record.schema_version === 4 ? supersessionFields : [])];
  ...
  if (![1, 2, 3, 4].includes(record.schema_version)) violations.push("BOOTSTRAP_CANDIDATE schema_version must be 1, 2, 3, or 4.");
  ...
  if (record.schema_version === 3 && record.validator_version !== "PHASE1-BOOTSTRAP-3") violations.push("BOOTSTRAP_CANDIDATE v3 validator_version mismatch.");
  if (record.schema_version === 4 && record.validator_version !== VALIDATOR_VERSION) violations.push(`BOOTSTRAP_CANDIDATE validator_version must be ${VALIDATOR_VERSION}.`);
  ...
  for (const field of ["manifest_sha256", "included_file_set_sha256", "scanner_report_sha256", "record_payload_sha256", ...(record.schema_version >= 2 ? bindingFields : []), ...(record.schema_version === 4 ? ["supersedes_candidate_manifest_sha256"] : [])]) requireSha(record[field], field, violations);
  if (record.schema_version === 4 && (typeof record.supersession_reason !== "string" || !record.supersession_reason.trim())) violations.push("BOOTSTRAP_CANDIDATE supersession_reason is required.");
  if (record.schema_version === 4 && record.breaking_governance_change !== "referenced_evidence_validation_required") violations.push("BOOTSTRAP_CANDIDATE breaking_governance_change must identify referenced_evidence_validation_required.");
  ...
  for (const field of ["task_id", "manifest_sha256", "included_file_set_sha256", "file_count", "scanner_report_sha256", "record_type", "assurance", ...bindingFields, ...(record.schema_version === 4 ? supersessionFields : [])]) {
