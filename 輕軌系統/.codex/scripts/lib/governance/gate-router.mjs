...
export const GATE_DEPENDENCY_CONTRACT = Object.freeze({
  ...
  gates: {
    bootstrap_candidate_review: ["candidate_gate_blockers", "referenced_evidence_validation"],
...
const allowedInputKeys = new Set(["candidate_gate_blockers", "referenced_evidence_validation", "human_commit", "steady_state_anchor", "migration_320_gate_blockers"]);
...
export function computeAuthoritativeGateMap(input = {}) {
  ...
  const evidence = input.referenced_evidence_validation ?? {};
  const evidenceCountsValid = Number.isInteger(evidence.required_count) && evidence.required_count > 0 && evidence.resolved_count === evidence.required_count && evidence.validated_count === evidence.required_count && evidence.invalid_count === 0 && evidence.unresolved_count === 0 && evidence.duplicate_count === 0;
  if (evidence.complete !== true || !evidenceCountsValid) candidateBlockers.push("REFERENCED_EVIDENCE_INCOMPLETE");
