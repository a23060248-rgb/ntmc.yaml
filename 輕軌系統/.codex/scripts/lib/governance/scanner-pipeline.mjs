...
export function validateDeclaredScanContract(contract, { findingRegistry, canonicalizationConfig, binaryOracle, binaryMagicRegistry, implementationMatrix, productionCaseManifest } = {}) {
  ...
  if (contract?.evidence_schema_registry_ref !== ".codex/governance/evidence-schema-registry.yaml" || !/^[A-F0-9]{64}$/.test(contract?.evidence_schema_registry_sha256 ?? "")) violations.push("SCAN_CONTRACT evidence-schema registry reference/hash mismatch.");
  if (contract?.referenced_evidence_policy_ref !== ".codex/governance/referenced-evidence-validation-policy.yaml" || !/^[A-F0-9]{64}$/.test(contract?.referenced_evidence_policy_sha256 ?? "")) violations.push("SCAN_CONTRACT referenced-evidence policy reference/hash mismatch.");
  ...
  if (implementationMatrix) {
    ...
    const capabilities = new Map((implementationMatrix.validation_capabilities ?? []).map((row) => [row[0], row]));
    for (const capability of ["referenced_evidence_resolution", "evidence_schema_registry", "candidate_gate_completeness"]) if (!capabilities.has(capability)) violations.push(`SCAN_CONTRACT implementation matrix lacks ${capability}.`);
  }
  return violations;
}
