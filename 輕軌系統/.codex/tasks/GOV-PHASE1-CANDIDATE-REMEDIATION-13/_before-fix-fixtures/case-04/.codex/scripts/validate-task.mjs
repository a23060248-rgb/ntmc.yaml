...
const requiredFiles = ["task-intent.yaml", "classification.yaml", "blueprint.yaml", "implementation-plan.md", "implementation-handoff.yaml", "artifact-manifest.yaml", "test-evidence.yaml", "security-evidence.yaml", "review-findings.yaml", "human-approval.yaml", "traceability.yaml", "final-summary.md"];
const schemaFiles = {
  ...
  "test-evidence.yaml": "test-evidence.schema.json",
  "security-evidence.yaml": "security-evidence.schema.json",
  "review-findings.yaml": "review-findings.schema.json",
...
async function validateBootstrapRecordRef({ projectRoot, recordRef, scannerReportRef, expected, governance, errors, gateReasons, schemaSet }) {
  try {
    ...
    for (const field of ["contract_sha256", "finding_registry_sha256", "canonicalization_config_sha256", "binary_oracle_config_sha256", "binary_magic_registry_sha256", "schema_set_sha256"]) {
...
export async function validateTask(taskId, options = {}) {
  ...
  const handoff = documents["implementation-handoff.yaml"], taskManifest = documents["artifact-manifest.yaml"], evidence = documents["test-evidence.yaml"];
  const securityEvidence = documents["security-evidence.yaml"], findings = documents["review-findings.yaml"], approval = documents["human-approval.yaml"], traceability = documents["traceability.yaml"];
  ...
  return { exitCode, calculatedGate, projection, targetGate, gateResults, errors, gateReasons: candidateReasons, candidateReasons, migrationReasons, effectiveScope, actorScope, ruleSelection, proofAssurance: BOOTSTRAP_ASSURANCE, typedDomainRouting };
}
