...
const [taskId, generatedAt, scannerReportRef, outputRef] = process.argv.slice(2);
if (!/^[A-Z][A-Z0-9-]{2,63}$/.test(taskId ?? "") || Number.isNaN(Date.parse(generatedAt)) || !scannerReportRef || !outputRef) throw new Error("task ID, ISO timestamp, scanner report ref, and output ref are required.");
