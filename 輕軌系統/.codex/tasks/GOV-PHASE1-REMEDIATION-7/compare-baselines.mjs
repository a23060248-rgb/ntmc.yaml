...
const types = ["governance-source", "frozen-history", "migration-320-task", "migration-320-external-evidence"];
...
console.log(`BASELINE_COMPARISON governance_modified=${byType["governance-source"].modified.length} governance_added=${byType["governance-source"].added.length} governance_deleted=${byType["governance-source"].deleted.length} frozen_identical=${payload.frozen_history_identical} m320_task_identical=${payload.migration_320_task_identical} m320_external_identical=${payload.migration_320_external_evidence_identical}`);
process.exit(payload.frozen_history_identical && payload.migration_320_task_identical && payload.migration_320_external_evidence_identical ? 0 : 1);
