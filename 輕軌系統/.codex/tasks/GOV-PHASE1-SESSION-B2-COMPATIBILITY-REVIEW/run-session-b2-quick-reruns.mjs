...
const summary={result:[syntax.every((i)=>i.pass),fixturesRun.status===0,concurrencyRun.status===0,scannerRun.status===0,integrationRun.status===0,bindingRun.status===0,loaderRun.status===0,loaderMutationRun.status===0,binaryRun.status===0].every(Boolean)?"PASS":"FAIL",syntax:"18/18",fixtures:`${fixtureSummary[2]}/${fixtureSummary[1]}`,concurrency:concurrency.passed,scanner:`${scanner.passed}/${scanner.total}`,integration:`${integration.passed}/${integration.total}`,reviewer_binding:`${binding.passed}/${binding.total}`,loader:`${loader.passed}/${loader.total}`,loader_mutations:`${loaderMutations.killed}/${loaderMutations.total}`,binary_mutations:`${binary.killed}/${binary.total}`};
