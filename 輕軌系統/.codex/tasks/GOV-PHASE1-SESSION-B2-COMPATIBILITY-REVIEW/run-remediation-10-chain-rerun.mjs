...
function scopeHash(scope){const copy=clone(scope);delete copy.scope_payload_sha256;const evidenceEntry=copy.allowed_files?.find((item)=>item.path.endsWith("/session-b2-security-evidence.yaml"));if(evidenceEntry)evidenceEntry.expected_sha256=null;return canonicalSha256(copy);}
...
const scope=await loadTask("session-b2-security-read-scope-final.json");
const evidence=await loadTask("session-b2-security-evidence.yaml");
...
const suites=[{suite:"contract-version-derivation",cases:contractCases},{suite:"exact-security-allowlist",cases:allowCases},{suite:"candidate-security-binding-chain",cases:chainCases}].map((item)=>({...item,total:item.cases.length,passed:item.cases.filter((c)=>c.result==="PASS").length,failed:item.cases.filter((c)=>c.result!=="PASS").length}));
const total=suites.reduce((sum,item)=>sum+item.total,0),passed=suites.reduce((sum,item)=>sum+item.passed,0);
const payload={schema_version:1,task_id:taskId,source_remediation_task:"GOV-PHASE1-REMEDIATION-10",execution_mode:"fresh_b2_direct_revalidation",production_contract_derived:true,total,passed,failed:total-passed,suites,result:passed===total?"PASS":"FAIL",completed_at:new Date().toISOString()};
await writeFile(path.join(taskDir,"remediation-10-chain-test-rerun.json"),`${JSON.stringify(payload,null,2)}\n`,{encoding:"utf8",flag:"w"});
console.log(JSON.stringify({result:payload.result,total:payload.total,passed:payload.passed,suites:payload.suites.map((item)=>({suite:item.suite,total:item.total,passed:item.passed}))}));
