...
const candidateManifestVerification = {
  ...
  a10_artifacts_in_candidate: included.some((item) => item.path.startsWith(".codex/tasks/GOV-PHASE1-L3-REVIEW-A10/")),
...
const expectedIds = [
  "A10-CODE-RUN-1F69FAA9-92B3-48C9-8D80-95463B5A1336", "A10-CODE-SESSION-0C9FD58E-5258-4BE9-B0C3-69AD36A1D3AE",
  "A10-SEC-RUN-085B8070-F7D7-4A30-B7B3-66D54FE2492F", "A10-SEC-SESSION-DD0A4AA4-A697-444D-BD0C-CC8AACF99C92",
  "A10-DOMAIN-RUN-E0486A9D-394A-4568-8257-CDB23E78159D", "A10-DOMAIN-SESSION-994938BC-6E74-4AA3-A226-2B2A29BB87D0"
];
async function historicalCollisions(relativePath, collisions) {
  ...
  for (const entry of await readdir(absolute, {withFileTypes: true})) {
    ...
    if (ref.startsWith(".codex/tasks/GOV-PHASE1-L3-REVIEW-A10")) continue;
    if (entry.isDirectory()) await historicalCollisions(ref, collisions);
    else if (entry.isFile()) {
      const text = (await readFile(await safeExistingPath(root, ref))).toString("utf8");
...
const assignmentVerification = {
  ...
  reviewer_run_and_session_ids_unique_within_a10: new Set(expectedIds).size === expectedIds.length,
...
console.log(`A10_CANDIDATE_VERIFY files=${included.length} scanner=${reportCheck.ok} schemas=${schemaSet.schema_set_sha256} assignments=${assignmentVerification.result}`);
