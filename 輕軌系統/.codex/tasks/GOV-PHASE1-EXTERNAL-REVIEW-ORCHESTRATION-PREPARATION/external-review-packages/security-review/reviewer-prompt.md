# GOV-PHASE1-B6EXT-SECURITY-REVIEW — Standalone External Top-Level Reviewer Prompt
...
- Act only as EXTERNAL_SECURITY_INTEGRITY_REVIEWER.
...
- Read only exact paths enumerated in `.codex/tasks/GOV-PHASE1-EXTERNAL-REVIEW-ORCHESTRATION-PREPARATION/external-review-packages/security-review/exact-read-scope.json`.
...
- Return exactly one JSON object conforming to `.codex/tasks/GOV-PHASE1-EXTERNAL-REVIEW-ORCHESTRATION-PREPARATION/external-review-packages/security-review/return-payload.schema.json`; no Markdown fence, natural-language summary, second outcome, or later correction.
- After returning the payload, stop immediately.

## Startup, before candidate content review

1. Read `.codex/tasks/GOV-PHASE1-EXTERNAL-REVIEW-ORCHESTRATION-PREPARATION/external-review-packages/security-review/assignment.json`, `package-manifest.json`, `package-verification.json`, `exact-read-scope.json`, `capability-artifact-matrix.json`, and `startup-contract.json`.
...
Read `.codex/tasks/GOV-PHASE1-EXTERNAL-REVIEW-ORCHESTRATION-PREPARATION/external-review-packages/security-review/review-requirements.json` and perform every mandatory check using only the exact scope. Treat any out-of-scope access or forbidden read as BLOCKER. Record every actually read path in `access_log.allowed_paths_read`.
