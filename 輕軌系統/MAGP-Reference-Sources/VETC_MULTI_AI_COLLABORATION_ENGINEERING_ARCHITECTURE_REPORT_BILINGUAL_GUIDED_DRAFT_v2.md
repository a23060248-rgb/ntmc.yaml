...
CHINESE_GUIDED_BILINGUAL — 中文負責主要說明；英文保留正式名稱、識別碼、欄位與狀態碼。
```
...
> **閱讀與報告方式：** 本版不將所有英文技術名稱全面翻譯。流程、角色與關鍵 Artifact 採「中文主說明＋英文正式名稱」；機器欄位、狀態碼、文件名稱與精確 identifier 保留英文，以兼顧口頭解釋與後續規範化。
...
```text
人類目標／正式來源（Human intent / source）
→ 明確人類決策（explicit Human decision）
→ 憲法層約束（Constitution）
→ 已採用架構（adopted Architecture）
→ 標準技術規格基線（canonical technical specification）
→ RFC 候選文件（RFC Candidate）
→ 獨立 RFC 審查（independent RFC review）
→ 人類採用 RFC（Human RFC adoption）
→ 執行套件（Execution Package）
→ 工頭有限工單（Foreman Work Order）
→ 工人實作（Worker implementation）
→ 工頭獨立證據驗證（Foreman independent evidence verification）
→ 整合（integration）
→ 黑箱功能資格驗證（black-box functional qualification）
→ 人類能力接受決策（Human capability acceptance）
```

```mermaid
flowchart LR
    H[人類擁有者<br/>Human Owner]
    C[憲法層約束<br/>Constitution]
    A[已採用架構<br/>Adopted Architecture]
    T[標準技術規格基線<br/>Canonical Technical Specification]
    RC[RFC 候選<br/>RFC Candidate]
    RV[獨立 RFC 審查<br/>Independent RFC Review]
    RA[人類採用 RFC<br/>Human RFC Adoption]
    EP[執行套件<br/>Execution Package]
    F[Codex 工頭<br/>Codex Foreman]
    W[Codex 工人<br/>Codex Worker]
    EV[證據驗證<br/>Evidence Verification]
    IN[整合<br/>Integration]
    Q[功能資格驗證<br/>Functional Qualification]
    HA[人類能力接受<br/>Human Capability Acceptance]
...
## 4. 以 Artifact 為媒介的協作（Artifact-mediated collaboration）
...
結構化來源包（Structured Source Bundle）
架構候選／決策紀錄（Architecture Candidate / Decision Record）
人類決策紀錄（Human Decision Record）
RFC 候選／已採用 RFC（RFC Candidate / Approved RFC）
審查與追蹤紀錄（Review and Trace Record）
執行套件（Execution Package）
執行契約（Execution Contract）
工頭逐字回讀（Foreman Literal Readback）
有限工單（Work Order）
工人結果包（Worker Result Package）
整合紀錄（Integration Record）
執行追蹤包（Execution Trace Bundle）
功能資格驗證證據（Functional Qualification Evidence）
```
...
### 6.1 單次執行控制（Execution Run Control）
...
### 6.2 RFC 依賴窗口（RFC Dependency Window）
...
### 6.4 功能資格驗證（Functional Qualification）

控制整體 capability claim。它以公開入口與外部可觀察行為驗證完整 Agent 路徑，不以單一 Worker、自寫測試或局部 pytest 綠燈替代。

## 7. P0 與 G1–G11 單次執行控制（Execution Run）

歷史 Execution Run Control 的固定順序為：

```text
P0   環境來源預檢（Environment Source Preflight）
G1   來源整理（Source Preparation）
G2   RFC 候選審查（RFC Review）
G3   人類 RFC 採用決策（Human RFC Approval）
G4   NotebookLM 來源同步確認（NotebookLM Source Sync）
G5   執行套件審查（Execution Package Review）
G6   工頭逐字回讀與可施工性檢查（Foreman Literal Readback + Feasibility）
G7   工人有限範圍實作（Worker Implementation）
G8   工頭整合與獨立重跑（Foreman Integration）
G9   Gemini 最終獨立監督（Gemini Final Supervision）
G10  架構符合性審查（Architect Conformance Review）
G11  人類接受或後續處置（Human Acceptance / Follow-up Decision）
```
...
## 8. RFC 依賴窗口（RFC Dependency Window）
...
```text
權威基線（Authority baseline）
來源清單（Source manifest）
依賴圖（Dependency graph）
保留邊界登錄表（Preserved-boundary register）
未決事項登錄表（Open-decision register）
禁止自行發明事項（Prohibited-invention register）
架構師工作包（Architect work package）
獨立驗證工作階段（Independent verification session）
人類處置包（Human disposition package）
```
...
需求（Requirements）
依賴（Dependencies）
公開與內部契約（Public and internal contracts）
失敗語意（Failure semantics）
禁止行為（Prohibited behavior）
必要測試（Required tests）
驗證結果（Verification result）
採用狀態（Adoption status）
下游影響（Downstream impact）
```
...
## 9. H0–H5 基礎治理序列（Foundational Control Sequence）

H0–H5 是 `PROPOSED_NEXT_PROTOCOL`，不是現行已採用 Gate。其功能如下。

### H0 — RFC 前準備度（Pre-RFC Readiness）
...
### H3 — 狀態／認知窗口（State / Cognition Window）
...
### H4 — 控制／執行期窗口（Control / Runtime Window）
...
### H5 — 基礎系列功能資格驗證（Foundational-Series Functional Qualification）

H5 不再判斷 RFC 文字是否完整，而是使用預先定義的黑箱測驗，從乾淨環境走完整能力路徑：

```text
乾淨環境安裝（install）
→ 啟動執行期系統（runtime start）
→ 輸入觀測資料（observation input）
→ 執行認知流程（cognition）
→ 產生動作請求（ActionRequest）
→ 經過安全決策（SafetyDecision）
→ 派送或明確安全不派送（dispatch / explicit safe non-dispatch）
→ 收集執行證據（ExecutionEvidence）
→ 建立執行週期紀錄（RuntimeCycleRecord）
```

H5 同時包含正常、拒絕、錯誤、fault injection 與多 cycle 情境。Critical Safety、dispatch 與 evidence chain 不得在最終資格驗證中被 mock 掉。

## 10. Codex 證據綁定施工（Evidence-bound Execution）

Codex 施工鏈為：

```text
已採用 RFC（Approved RFC）
→ 執行套件（Execution Package）
→ 工頭逐字回讀（Foreman Literal Readback）
→ 工頭可施工性檢查（Foreman Feasibility）
→ 有限工單（Limited Work Order）
→ 工人實作（Worker Implementation）
→ 工人結果包（Worker Result Package）
→ 工頭獨立重跑（Foreman Independent Rerun）
→ CI／整合證據（CI / integration evidence）
→ 工單接受（Work Order Acceptance）
```

Execution Contract 的逐字回讀用來證明 Foreman 接收到的是指定內容，而不是自行摘要後的版本。逐字回讀通過不代表可施工；Foreman 還必須檢查檔案、工具、權限、依賴、介面、衝突與可拆分性。

Work Order 必須指定下列機器可讀欄位；英文欄位名稱保留，以便後續模板與自動化直接使用：
...
| Figure | 圖名 |
|---|---|
| Figure 1 | 從權威到合格能力總覽（Authority-to-Qualified-Capability Overview） |
| Figure 2 | 六角色權限與交接（Six-role Authority and Handoff） |
| Figure 3 | 原始 P0／G1–G11 單次執行控制 |
| Figure 4 | RFC 依賴窗口與 H0–H5 |
| Figure 5 | Codex 證據綁定施工 |
| Figure 6 | 驗證與退回路由 |
| Figure 7 | 端到端追蹤 |
...
## 14. 權威（Authority）、Artifact 與狀態分離

### 14.1 權威層級

```mermaid
flowchart TD
    HD[明確人類決策<br/>Explicit Human Decision]
    C[憲法層約束<br/>Constitution]
    A[已採用架構<br/>Adopted Architecture]
    T[標準技術規格基線<br/>Canonical Technical Specification]
    R[已採用 RFC<br/>Approved RFC]
    E[執行套件<br/>Execution Package]
    WO[有限工單<br/>Work Order]
    CODE[程式實作<br/>Implementation]
    EVD[執行證據<br/>Execution Evidence]
    Q[資格驗證證據<br/>Qualification Evidence]
...
### 15.1 Figure 2 — 六角色權限與交接（Six-role Authority and Handoff）

```mermaid
flowchart TD
    H[人類擁有者 Human Owner<br/>採用／退回／延後／拒絕]
    SA[GPT 助理 GPT Assistant<br/>來源整理]
    AR[GPT 架構師 GPT Architect<br/>架構與 RFC 候選]
    SU[NotebookLM / Gemini 監督者<br/>獨立審查與追蹤]
    F[Codex 工頭 Codex Foreman<br/>回讀／可施工性／工單／整合]
    W[Codex 工人 Codex Worker<br/>有限範圍實作]
    EV[執行與資格驗證證據<br/>Execution / Qualification Evidence]
...
## 16. 來源清單與環境預檢（Source Manifest / Environment Preflight）
...
## 17. 原始 P0／G1–G11 單次執行控制（Execution Run Control）
...
```mermaid
flowchart TD
    P0[P0 環境來源預檢<br/>Environment Source Preflight] --> A[G1 來源整理<br/>Source Preparation]
    P0 -- 需要同步 SYNC REQUIRED --> P1[Human 更新專案資料來源]
    P1 --> P0

    A --> B[G2 RFC 候選審查<br/>RFC Review]
    B -- 退回 RETURN --> A2[GPT Architect / Assistant 修訂]
    A2 --> B
    B -- 通過 PASS --> C[G3 人類 RFC 採用決策<br/>Human RFC Approval]
    C -- 退回 RETURN --> A2
    C -- 已採用 APPROVED --> D[G4 NotebookLM 來源同步<br/>Source Sync]
    D -- 過期 OUTDATED --> D2[更新與重新驗證]
    D2 --> D
    D -- 通過 PASS --> E[G5 執行套件審查<br/>Execution Package Review]
    E -- 退回 RETURN --> E2[GPT Architect 修訂執行套件]
    E2 --> E
    E -- 通過 PASS --> F[G6 工頭逐字回讀與可施工性<br/>Literal Readback + Feasibility]
    F -- 阻擋 BLOCKED --> F2[Gemini 整理完整退回資料]
    F2 --> E2
    F -- 通過 PASS --> G[G7 工人有限實作<br/>Worker Implementation]
    G --> H[G8 工頭整合與重跑<br/>Foreman Integration]
    H -- 工單修正 WORK-ORDER FIX --> G
    H -- 完成 COMPLETE --> I[G9 Gemini 最終監督<br/>Final Supervision]
    I -- 實作退回 --> H
    I -- 架構退回 --> F2
    I -- 通過 PASS --> J[G10 架構符合性審查<br/>Architect Conformance Review]
    J -- 退回 RETURN --> E2
    J -- 通過 PASS --> K[G11 人類接受／後續處置<br/>Human Acceptance / Follow-up]
```
...
## 19. RFC 依賴窗口協定（Dependency Window Protocol）

### 19.1 Figure 4 — H0–H5 與 Window 路徑

```mermaid
flowchart TD
    H0[H0 RFC 前準備度<br/>Pre-RFC Readiness]
    H1[H1 基礎窗口<br/>Foundation Window]
    H2[H2 感知窗口<br/>Perception Window]
    H3[H3 狀態／認知窗口<br/>State / Cognition Window]
    H4[H4 控制／執行期窗口<br/>Control / Runtime Window]
    H5[H5 基礎系列功能資格驗證<br/>Foundational-Series Qualification]
    T[後續 RFC：分級／觸發分類<br/>Tier / Trigger Classification]
    ...
    subgraph WindowCycle[每個 H1-H4 窗口的共同流程]
        WC[窗口控制包<br/>Window Control]
        AC[架構師 RFC 候選<br/>Architect RFC Candidates]
        VR[逐 RFC＋跨 RFC 驗證<br/>Per-RFC + Cross-RFC Verification]
        HD[人類窗口處置<br/>Human Window Disposition]
        WO[工頭工單<br/>Foreman Work Orders]
        IM[工人實作<br/>Worker Implementation]
        IN[窗口整合<br/>Window Integration]
...
下列欄位保留英文 identifier；報告時可概括為「窗口身分、進入條件、依賴、共同邊界、逐 RFC 結果、Human disposition 與退出條件」：
...
| 控制點（Control point） | 候選內容（Candidate content） | 主要協作風險（Core risk） |
|---|---|---|
...
## 20. 執行套件、逐字回讀與可施工性（Execution Package / Literal Readback / Feasibility）
...
## 21. 工單與工人結果契約（Work Order / Worker Result Contract）

### 21.1 Work Order schema

下列為正式工單欄位；中文說明已在本章前文提供，欄位本身維持英文以避免後續實作時產生名稱漂移：
...
## 22. 證據綁定完成（Evidence-bound Completion）

### 22.1 Figure 5 — 完成證據鏈

```mermaid
flowchart LR
    R[需求識別碼<br/>Requirement ID]
    C[程式位置<br/>Code Location]
    T[測試位置<br/>Test Location]
    N[負向測試<br/>Negative Test]
    CMD[命令執行結果<br/>Command Result]
    SC[範圍／禁止項掃描<br/>Scope / Forbidden Scan]
    WR[工人結果<br/>Worker Result]
    FR[工頭獨立重跑<br/>Foreman Independent Rerun]
    CI[CI／整合證據<br/>CI / Integration Evidence]
    AC[已接受工單<br/>Accepted Work Order]
...
```mermaid
flowchart TD
    D[發現問題<br/>Detected Problem]
    I{局部實作缺陷？<br/>Local implementation defect?}
    WO{工單或測試缺陷？<br/>Work-order / test defect?}
    R{RFC 缺漏或歧義？<br/>RFC missing / ambiguous?}
    H{需要 Human 選擇？<br/>Human selection required?}
    A{涉及權威／ownership／ABI／topology／Safety／dispatch？}
    E{必要環境不可用？<br/>Environment unavailable?}

    D --> I
    I -- 是 Yes --> W[Worker 修正]
    I -- 否 No --> WO
    WO -- 是 Yes --> F[Foreman 建立修正工單]
    WO -- 否 No --> R
    R -- 是 Yes --> AR[Architect 修訂 RFC]
    R -- 否 No --> H
    H -- 是 Yes --> HU[Human decision]
    H -- 否 No --> A
    A -- 是 Yes --> AA[Architecture／Human 升級處理]
    A -- 否 No --> E
    E -- 是 Yes --> BE[BLOCKED_ENVIRONMENT]
    E -- 否 No --> X[調查 evidence 或 source]
```

### 23.2 退回矩陣

| 問題（Problem） | 結果（Result） | 退回對象（Return target） | 保留結果（Preserved results） |
|---|---|---|---|
...
| 驗證階段（Verification stage） | 審查者（Reviewer） | 輸入 | 輸出 | 不足以證明 |
|---|---|---|---|---|
...
## 25. 功能資格驗證（Functional Qualification）

### 25.1 Qualification Plan schema

下列欄位用來固定「驗證哪個能力、在哪個環境、用哪些情境、保留什麼證據、何時才可 PASS」；identifier 保留英文：
...
## 26. 重量級驗證、分級與觸發條件（Heavy Verification / Tier / Trigger）

### 26.1 Verification level

| 等級（Level） | 控制方式（Control） | 典型用途（Typical use） |
|---|---|---|
...
## 28. 端到端追蹤（End-to-End Traceability）

### 28.1 Figure 7 — 完整追蹤鏈

```mermaid
flowchart LR
    HD[人類決策<br/>Human Decision]
    HC[憲法條款<br/>Constitution Clause]
    AD[架構決策<br/>Architecture Decision]
    TR[技術需求<br/>Technical Requirement]
    RR[RFC 需求<br/>RFC Requirement]
    WO[工單需求<br/>Work Order Requirement]
    C[程式碼<br/>Code]
    T[測試<br/>Test]
    EE[執行證據<br/>Execution Evidence]
    QR[資格驗證結果<br/>Qualification Result]
    HA[人類能力接受<br/>Human Capability Acceptance]
...
| 追蹤連結（Trace edge） | 必要識別（Required identity） |
|---|---|
...
## 29. 現行、歷史與候選內容的整合邊界（Current / Historical / Proposed）
