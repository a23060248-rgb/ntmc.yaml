...
async function main() {
  ...
  assert.match(firstMigrationRun, /"ledgerRows": 25/);
  ...
  assert.match(secondMigrationRun, /"verified": 24/);
  assert.match(secondMigrationRun, /"applied": 0/);

  const freshSummary = await querySummary(freshUrl);
  assert.equal(freshSummary.ledger_rows, 25);
  assert.equal(freshSummary.applied_rows + freshSummary.baselined_rows, 25);
  ...
  console.log(JSON.stringify({
    ...
    migrationSecondRun: { verified: 24, applied: 0 },
    freshSummary,
