...
const { claimIdempotency, completeIdempotency } = require("../services/idempotencyService");

const router = express.Router();
router.use(resolveAuth);

const INVENTORY_POST_ROLES = ROLE_POLICIES.INVENTORY_POST;
const CONSUME_ROLES = ROLE_POLICIES.INVENTORY_CONSUME;
...
function idempotencyKey(req) {
  return cleanText(req.get("Idempotency-Key") || req.body?.idempotencyKey);
}

function postingPayload(body) {
  const { idempotencyKey: ignored, ...payload } = body || {};
  return payload;
}
...
async function assertEnoughStock(client, { materialId, warehouse, bin, stockStatus, qty }) {
  await client.query(
    ...
    [`inventory:${materialId}:${warehouse.id}:${stockStatus}`]
  );
...
async function createInventoryMovement(body, forcedMovementType, context = {}) {
  ...
  return withTransaction(async (client) => {
    const claim = await claimIdempotency(client, {
      key: context.idempotencyKey || body.idempotencyKey,
      operationCode: `INVENTORY_${movementType}`,
      payload: postingPayload(body),
      actorId: context.actorId,
    });
    if (claim.replay) return { ...claim.response, idempotentReplay: true };
    ...
    await completeIdempotency(client, claim, movement);
...
async function consumeMaterialWithClient(client, body, context = {}) {
  const partNo = normalizePartNo(body.partNo);
  if (!partNo) throw httpError(400, "partNo is required");
  const qty = requirePositiveQty(body.qty);
  ...
  const claim = await claimIdempotency(client, {
    key: context.idempotencyKey || body.idempotencyKey,
    operationCode: "INVENTORY_CONSUME",
    payload: postingPayload(body),
    actorId: context.actorId,
  });
  if (claim.replay) return { ...claim.response, idempotentReplay: true };
  ...
  await completeIdempotency(client, claim, consumption);
  return { ...consumption, idempotentReplay: false };
}

async function consumeMaterial(body, context = {}) {
  return withTransaction(async (client) => consumeMaterialWithClient(client, body, context));
}
...
async function movementResponse(req, res, movementType) {
  ...
  res.status(movement.idempotentReplay ? 200 : 201).json({ movement });
}
...
router.post(
  "/movements",
  requireRoles(...INVENTORY_POST_ROLES),
  asyncHandler(async (req, res) => {
...
router.post(
  "/consume",
  requireRoles(...CONSUME_ROLES),
  asyncHandler(async (req, res) => {
    const consumption = await consumeMaterial(req.body || {}, {
      idempotencyKey: idempotencyKey(req),
      actorId: actorId(req),
    });
    res.status(consumption.idempotentReplay ? 200 : 201).json({ consumption });
  })
);

module.exports = router;
module.exports.consumeMaterialWithClient = consumeMaterialWithClient;
