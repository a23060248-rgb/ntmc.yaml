...
export function AttachmentEditor({ items, editable, saving, onChange, onSave }: { items: TemplateAttachment[]; editable: boolean; saving: boolean; onChange(value: TemplateAttachment[]): void; onSave(): void }) {
  ...
  return <div className="attachment-studio">
    ...
    </> : <div className="empty-inline">尚未建立附件。</div>}{editable ? <button className="primary-button studio-save" type="button" disabled={saving} onClick={onSave}><Save size={15} />儲存附件設定</button> : null}</section>
  </div>;
}
