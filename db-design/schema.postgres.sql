...
CREATE TABLE pm_template_check_item (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  pm_template_id uuid NOT NULL REFERENCES pm_template(id) ON DELETE CASCADE,
...
CREATE INDEX idx_pm_template_check_item_template
  ON pm_template_check_item(pm_template_id, is_active, sort_order);

CREATE INDEX idx_pm_template_check_item_section
...
CREATE TABLE pm_work_order_check_result (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_pm_work_order_check_result_order
  ON pm_work_order_check_result(work_order_id);

CREATE INDEX idx_pm_work_order_check_result_item
  ON pm_work_order_check_result(check_item_id);

CREATE INDEX idx_pm_work_order_check_result_status
